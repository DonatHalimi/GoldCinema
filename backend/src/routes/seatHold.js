...
router.post(
    ...
    [
        ...
        body('seatIds').isArray({ min: 1, max: 8 }).withMessage('Select between 1 and 8 seats.'),
    ],
    async (req, res, next) => {
        try {
...
router.post(
    ...
    requireAuth,
    requireVerified,
    [body('holdId').notEmpty().withMessage('holdId is required.')],
    async (req, res, next) => {
        try {
            const errors = validationResult(req);
            if (!errors.isEmpty()) {
...
router.post('/release-hold', requireAuth, async (req, res, next) => {
    try {
...
router.get('/holds/:id', requireAuth, async (req, res, next) => {
    try {
